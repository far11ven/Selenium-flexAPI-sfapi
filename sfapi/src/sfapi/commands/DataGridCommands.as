...
package sfapi.commands
{
	...
	public class DataGridCommands
	{
		...
		public function doFlexSelectDataGridRow(id:String, rowNum:String):String

		{
			
			var child:Object = appTreeParser.getElement(id);
			
			var selectRowNum:int = int (rowNum);
						
			try{
				
			child.selectedIndex = selectRowNum-1;
			
			}
			catch (e:Error)
			{
				var result:String = "ERROR: Widget '" + id + "': " + e.message;
		...
		public function getFlexDataGridCellElement(value:String):String
		{
			
			var args:Array = value.split(",");
			var id:String = args[0];
			var dataGridTotalRows:int = args[1];
			var dataGridTotalCol:int = args[2];
			var row:int = args[3];
			var col:int = args[4];
			...
			//getting name of child node
			
			var childPath:String = child.getChildAt(2).getChildAt(2+((row+1)*dataGridTotalCol)+col).getChildAt(0);
			
			var args1:Array = childPath.split(".");
			var myChild:String = args1[(args1.length)-1];
			var childlength:String = String (args1.length);
